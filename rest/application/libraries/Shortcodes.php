...
class Shortcodes
{
    ...
    public function parse ($content_id,$str,$language_slug)
    {
        ...
        preg_match_all('/\[load:([a-zA-Z0-9-_: |=\.]+)]/', $str, $shortcodes);
        ...
        if (count($shortcode_array)) {
            ...
            $path = realpath('./application/models/') . '/'; 
            foreach ($shortcode_array as $search => $shortcode) {
                $shortcode_model = $shortcode['code'];
				/* if (array_key_exists('type', $shortcode['params']) && $shortcode['params']['type']=='form') {
					 $path = realpath('./application/views/') . '/'; 
					 var_dump($this->_ci->load->view('contact_view')); 
					  $shortcode_model ='contact_view.php'; 
					 //  $this->_ci->load->view(APPPATH.'/views/public/'.$shortcode_model);
					 } */
				 if (file_exists($path . $shortcode_model . '.php') && is_file($path . $shortcode_model . '.php')) {
                    $this->_ci->load->model('/' . $shortcode_model); 
                    $str = str_replace($search, $this->_ci->$shortcode_model->run($shortcode['params'],$content_id,$language_slug), $str); 
                }
